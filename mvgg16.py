...
def VGG16(input_shape, classes, standard_input, alpha=1, expansion=6, stride=2):
    img_input = Input(shape=input_shape)
    x = img_input
    # 第一个卷积部分
    x = _inverted_res_block(x, filters=64, alpha=alpha, stride=1,
                        expansion=expansion)
    x = _inverted_res_block(x, filters=64, alpha=alpha, stride=stride,
                        expansion=expansion)
    x = _inverted_res_block(x, filters=128, alpha=alpha, stride=1,
                        expansion=expansion)
    x = _inverted_res_block(x, filters=128, alpha=alpha, stride=stride,
                        expansion=expansion)
    x = _inverted_res_block(x, filters=256, alpha=alpha, stride=1,
                        expansion=expansion)
    x = _inverted_res_block(x, filters=256, alpha=alpha, stride=1,
                        expansion=expansion)
    x = _inverted_res_block(x, filters=256, alpha=alpha, stride=stride,
                        expansion=expansion)
    x = _inverted_res_block(x, filters=512, alpha=alpha, stride=1,
                        expansion=expansion)
    x = _inverted_res_block(x, filters=512, alpha=alpha, stride=1,
                        expansion=expansion)
    x = _inverted_res_block(x, filters=512, alpha=alpha, stride=stride,
                        expansion=expansion)
    x = _inverted_res_block(x, filters=512, alpha=alpha, stride=1,
                        expansion=expansion)
    x = _inverted_res_block(x, filters=512, alpha=alpha, stride=1,
